...
export class BotService implements OnModuleInit {
  ...
  startBot() {
    this.bot.start(async ctx => {
      ...
      await ctx.reply(
        'Assalomu alaykum!\n\nIsm va familiyangizni lotin alifbosida, pasport(ID) yoki tug‘ilganlik guvohnomasiga mos ravishda yozing:',
      );
    });
    ...
    // Jins tanlash action handleri
    this.bot.action(/gender_(male|female)/, async ctx => {
    ...
    // Ism familiya va boshqa step-lar
    this.bot.on('text', async ctx => {
      ...
      if (step === 'askName') {
        ...
        this.userSteps.set(ctx.chat.id, 'askGender');
        return ctx.reply(
          'Jinsingizni tanlang:',
      ...
      if (step === 'askBirthDate') {
        ...
        const dateRegex = /^\d{2}\.\d{2}\.\d{4}$/; // Faqat DD.MM.YYYY
        if (!dateRegex.test(birthDateStr)) {
          return ctx.reply(
            "Iltimos, tug'ilgan kuningizni faqat DD.MM.YYYY formatida kiriting (masalan: 02.10.2010):",
          );
        }
